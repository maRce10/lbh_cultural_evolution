...
x <- c( "pbapply", "parallel", "ggplot2", "ape", "seqinr", "ips","gtools", "seqmagick", "ggmsa", "revbayes/RevKnitr"
        )
...
opt.algn <- list.files(path = "./data/processed/mafft_alignments", pattern = "alignment.fa$", full.names = TRUE)

opt.algn <- grep("optimal", opt.algn, value = TRUE)
...
out <- lapply(leks, function(x){
  ...
  cll <- paste0("mafft --anysymbol --keeplength --add ", grep(paste0("new_", x), rest.seqs, value = TRUE)," --reorder ", grep(paste0(x,"_optimal_early_old_alignment"), opt.algn, value = TRUE), " > ./data/processed/mafft_alignments/", x,"_optimal_all_new_aligment.fa")
  
  system(cll)
  
  cll <- paste0("mafft --anysymbol --keeplength --add ", grep(paste0("old_", x), rest.seqs, value = TRUE)," --reorder ", grep(paste0(x, "_optimal_early_old_alignment"), opt.algn, value = TRUE), " > ./data/processed/mafft_alignments/", x,"_optimal_all_old_aligment.fa")
...
all.algs <- grep("all.equal", all.algs, value = TRUE, invert = TRUE)


for (j in leks){
  
  
  sub.algs <- grep(j, all.algs, value = TRUE)
   #Convert fasta alignment to nexus
# all
  # new
  cll <- paste0(" perl ./source/convertfasta2nex.pl ./data/processed/mafft_alignments/", grep("all_new", sub.algs, value = TRUE), " > ",  "./data/processed/nexus/", j, "_all_new.nex")

 system(cll)  

 #all old
  cll <- paste0(" perl ./source/convertfasta2nex.pl ./data/processed/mafft_alignments/", grep( "all_old", sub.algs, value = TRUE), " > ",  "./data/processed/nexus/", j, "_all_old.nex")

 system(cll)  
   
 
 # early new
  cll <- paste0(" perl ./source/convertfasta2nex.pl ./data/processed/mafft_alignments/", grep("early_new", sub.algs, value = TRUE), " > ",  "./data/processed/nexus/", j, "_early_new.nex")

 system(cll)  

 # early old
  cll <- paste0(" perl ./source/convertfasta2nex.pl ./data/processed/mafft_alignments/", grep( "early_old", sub.algs, value = TRUE), " > ",  "./data/processed/nexus/", j, "_early_old.nex")
...
yrs$song.type.year <- paste(yrs$song.type, yrs$year, sep = "_")


alignment	<- c("optimal", "ER") # ER = equal rates
...
out <- lapply(1:nrow(grd), function(x){
 ...
 nx <- paste0("./data/processed/nexus/", grd$lek[x], "_", grd$use_all_fossils[x], "_", grd$use_old_fossils[x], ".nex")
  ...
  tsv <- paste0("./data/processed/revBayes/fossils/", grd$lek[x], "_taxa_", grd$use_all_fossils[x], "_", grd$use_old_fossils[x], ".tsv")
    ...
    fl.nm.rb <- paste0(grd$lek[x], "_", grd$use_all_fossils[x], "_", grd$use_old_fossils[x],".Rev")
     
    writeLines(templ, con = paste0("./source/", fl.nm.rb))
