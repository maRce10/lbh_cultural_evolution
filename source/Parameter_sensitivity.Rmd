---
title: "Sensitivity of song diversification and evolution inference"
...
Here we look at the effects of alignment strategies, clock models, fossil records and sampling on diversification parameters, as well as the effects of alignment strategies on the parameters of models of song evolution.
...
```{r useful-vars, eval=TRUE, echo=TRUE}
...
# Diversification rates and origin times

## Alignment strategy

To compare among alignments, we will focus on the results under a relaxed clock model, sampling all historical records (including identical songs in consecutive years) and the entire historical record, even if very gapped.

First, we select and merge output files
```{r Uexp-all-old-data}
...
We plot diversification parameters by lek and alignment.
```{r plot-div-aln-h}
...
We might want to quantify sensitivity to the alignment strategy. We can see how much of the posterior distribution of a parameter under a "query" alignment falls outside the 95% HPD interval of a "target" alignment. A higher non-overlap means the alignments produce different results. If query A falls within target B, but a lot of query B falls outside target A, then the alignment B produces a broader (more uncertain) posterior than alignment A.
...
## Clock model
Are diversification parameters sensitive to the clock model?

### For PRANK
```{r plot-div-h-clock-prank}
...
## Use of the historical record
Are diversification parameters sensitive to a gappy historical record? What about ignoring historical songs (fossils) altogether?

### For PRANK
Assuming a relaxed clock and sampling all fossil records
We can only use CCE HC1 and SUR for new vs old and BR1 TR1 and SUR for fossils vs no fossils
```{r plot-div-h-record-prank}
...
## Sampling strategy of historical records

Are diversification parameters sensitive to consecutive sampling of the same songs

### For PRANK
Assuming a relaxed clock and using old fossils when available
```{r plot-div-h-sampling-prank}
...
Assuming a relaxed clock and using old fossils when available.
```{r plot-div-h-sampling-agnostic}
...
# Song evolution parameters

We want to know if the parameters of the substitution model are sensitive to the alignment strategy.
...
This could be helpful to see if there is a lek or parameter that is particularly sensitive to the alignment strategy.

```{r evol-aln-sensitivity}
