...
# Each one was run for both different rates for each branch and a single rate (global model)

```{r revbayes loop regular prank and global model, eval = FALSE, echo = TRUE}
...
for (x in 1:nrow(grd)) {
  ...
  templ[12] <-
    if (grd$branch_rate_model[x] == "global")
      'source("./source/Specimens_control_global.Rev")'
  else
...
```{r revbayes loop regular prank and global model DEPRECATED, eval = FALSE, echo = TRUE}
...
# copy files
copy_res <- file.copy(from = files_to_copy, to = file.path("./output/most_recent_revbayes_models", basename(files_to_copy)), overwrite = FALSE)
