...
Record.labs <- c("Recent records only", "Including oldest records", "Only extant")
names(Record.labs) <- c("new", "old", "no.fossils")
...
ggs <- lapply(params, function(x){
  ...
  dat$fossils[dat$fossils == "full.process"] <- "all"
...
p3_results$Fossils <- factor(p3_results$Fossils, levels = c("all", "early"))
