...
# method = "last_year" for leaving only the tips found in the last year that a lek was sampled (for all fossil models) and "shared" to leave only those tips shared by all trees (a proportion given by "prop.trees")
tree_prunner <- function(trees, method = "last_year", prop.trees = 0.9){
...
custom_treespace <- function (multi_list, n.points = 100, thinning = 5000, method = "last_year"){
  ...
  # prune trees
  alltrees <- tree_prunner(trees = alltrees, method)

 if (method == "shared"){
   # extract labels
  model <- sapply(alltrees, "[[", "model")
  mods <- split(names(model), model)

  # get subset of n.points for each list
  mods <- lapply(mods, function(x){
    if (length(x) > n.points)
    x <- x[round(seq(from = 1, to = length(x), length.out = n.points), 0)]
    return(x)
    })

  mods <- unlist(mods)
  alltrees <- alltrees[names(model) %in% mods]
  }
