...
author: <center><font size="4"><a href="http://marceloarayasalas.weebly.com/">Marcelo Araya-Salas</a> & Beatriz Willink</font></center>
...
diag_stats <- function(x) {
  ...
  out <- lapply(mods, function(y){
    ...
    chain <- mcmc(data = temp , start = 1, end = length(temp[,1]), thin = 1)
...
runs <- pblapply(unique(base_models), cl = parallel::detectCores() - 1, function(x) try(diag_stats(x), silent = TRUE))
...
```{r plot traces, eval = FALSE, fig.height = 3.3, fig.width = 30, warning = FALSE, message = FALSE, dpi = 30}
...
# loop to make plots
out <- lapply(base_models[1:4], function(x){
  
  mfrow <- 
  
  jpeg(filename = file.path("./output/MCMC_diagnostic_plots", paste0(gsub("_([[:digit:]]+)", "", x), ".jpeg")), width = 1200, height = 1200)
  # info on base model    
