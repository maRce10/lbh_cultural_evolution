...
```{r load-packages, message=FALSE, results='hide', eval=TRUE, echo=FALSE}
x <-
  c(
    "tidyr",
    "ggplot2",
...
```{r sampling-times-plot, echo=FALSE}
...
<!--this is my proposal for fig 1, we can put it elsewhere, but I thought about it when I was here--> 
...
### Model reliabilty 
<!--can be renamed-->
```{r effect-size-plots, echo=FALSE}
...
Sampling.labs <- c("all historical", "earliest record", "none")
...
ggplot(data = p3_results[p3_results$Statistic == x, ], aes(
  ...
  scale_size_manual(values = c(3, 5)) + 
    scale_alpha_manual(values = c(0.44, 1)) +
    scale_colour_manual(
...
```{r plot-invar-segragating, echo=FALSE}
ggs2$`Number Invariant Sites`
ggs2$`Segregating-Sites`

ggs2$`Max Invariant Block Length`
ggs2$`Max Variable Block Length`
```
...
```{r  plot-max-pairwise, echo=FALSE}
ggs2$`Max Pairwise Difference`
```

```{r plot-max-pairwise-values, echo=TRUE, warning=FALSE}
...
```{r plot-min-pairwise, echo=FALSE}
ggs2$`Min Pairwise Difference`
```
...
```{r plot-genetic-diversity, echo=FALSE}
ggs2$Theta
ggs2$`Tajima-D`
ggs2$`Tajima-Pi`
```
...
```{r read-ML-table, include=FALSE, eval=TRUE}
source("../source/ML.table.R", local = knitr::knit_global())
```

```{r plot-ML-diff, warning=FALSE, echo=FALSE}
ML.plot
```
...
```{r source-parameter-sensitivity, include=FALSE}
